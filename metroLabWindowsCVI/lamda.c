	...
	if (FileSelectPopup ("c:\\Users\\utm\\Documents", "*.csv", "", "Seleccione el archivo de CLASES existentes", 
		...
		switch (k)
		{
		  ...
		  case 1:																	//usadas; dados los parametros proporcionados por
			dataSubjects = (double **) malloc ((Nsubjects + 1) * sizeof(double *));		//matrixClasesConfig().   
			for (j = 0; j < Nsubjects + 1; j++)
				dataSubjects[j] = (double *) malloc (Ndescriptors * sizeof(double));        
			...
			if (err = loadFile(pathFile2, dataSubjects, Nsubjects, Ndescriptors) >= 0)	//Cargamos el archivo de señales de entrada
			{
				MessagePopup ("Error de acceso", errorMap[err]);
				return 0;
			}
			...
			{										// entrada. De otro modo, Nsubjects seria indefinido, durante el tiempo que dure la prueba.
				...
				for (j = 0; j < vacio; j++)
					for (k = 0; k < Ndescriptors - 1; k++)
						MADs[j][k] = pow(clasesROs[j][k + 1], dataSubjects[i][k+1]) * pow((1 - clasesROs[j][k + 1]), 1 - dataSubjects[i][k+1]);	
			...
			// obtenemos las GADs					 //de la clase.
				...
				if (maxI == nic) 									//crear nueva clase:
				{
					...
					for (j = 1; j < Ndescriptors; j++)
						clasesROs[vacio][j] = 0.5 + ((dataSubjects[i][j] - 0.5)/ 2);   
				...
				else  //reforzar aprendizaje de una clase existente:
				{
					...
					for (j = 1; j < Ndescriptors; j++)			//Entrenamos la clase maxI con el nuevo elemento.
						clasesROs[maxI][j] = clasesROs[maxI][j] + ((dataSubjects[i][j] - clasesROs[maxI][j])/clasesROs[maxI][0]); 
...
/// HIFN  LAMDA_  On Line Learning(aprendizaje en tiempo real). 
...
int LAMDA_OLL (double *normData, int *claseReconocida, size_t normDataVectorSize)	
{
	...
	FILE *clasesFile;
		...
		if (normDataVectorSize != Ndescriptors)									    	//Verificamos si las dimenciones corresponden, en
